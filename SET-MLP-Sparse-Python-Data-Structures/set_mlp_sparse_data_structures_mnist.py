...
if __name__ == "__main__":
    ...
    # one hot encoding
    noClasses = np.max(Y_train)+1
    Y = np.zeros((Y_train.shape[0], noClasses))
    Y[np.arange(Y_train.shape[0]), Y_train.tolist()] = 1
    y = np.zeros((Y_test.shape[0], noClasses))
    y[np.arange(Y_test.shape[0]), Y_test.tolist()] = 1
    ...
    # create SET-MLP
    set_mlp = SET_MLP((X_train.shape[1], 1000, 1000, 1000, Y.shape[1]), (Relu, Relu, Relu, Sigmoid), epsilon=20)

    # train SET-MLP
    set_mlp.fit(X_train, Y, X_test, y, loss=MSE, epochs=100, batch_size=32, learning_rate=0.01, momentum=0.9, weight_decay=0.0002, zeta=0.3, testing=True, save_filename="Results/set_mlp.txt")

    # test SET-MLP
    accuracy,_=set_mlp.predict(X_test,y,batch_size=1)
